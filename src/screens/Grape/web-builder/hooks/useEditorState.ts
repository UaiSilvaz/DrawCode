...
import type { CanvasPage, EditorInstance, CanvasElementNode, SavedGrapeProjectData } from '../builder-core';
import { normalizeSavedActivePageIndex, normalizeSavedCanvasPages } from '../builder-core';

export function useEditorState(
  initialProjectId?: string,
  initialProjectName?: string,
  initialProjectData?: SavedGrapeProjectData | null,
) {
  const initialPages = normalizeSavedCanvasPages(initialProjectData);
  const initialActivePageIndex = normalizeSavedActivePageIndex(
    initialProjectData?.activePageIndex,
    initialPages.length,
  );
  ...
  const [activePageIndex, setActivePageIndex] = useState(initialActivePageIndex);
